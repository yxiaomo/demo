...
var mtimer = (function (elements,opts) {
		var defaults = {
			dateStart : new Date(),
			dateNum : 14,
			timeStart : 8,
		...
		for(var optsNum in  opts){
			defaults[optsNum] = opts[optsNum];
		}
		
		var option = defaults;

		var $alertBox = document.querySelector("#timeslot"),
			itemHeight = 48;
		var picker = {
			init : function(status){
				...
				var container = document.querySelector('.timeslot_alert .date_select_box.active .mt_poppanel'),
				mpDate = document.querySelector('.timeslot_alert .date_select_box.active .mt_date'),
				mpTime = document.querySelector('.timeslot_alert .date_select_box.active .mt_time');
				...
				mpDate.querySelector('ul').innerHTML += dateStr;
				...
				for(var j=0; j<option.timeNum; j++){
					var t = option.timeStart + j,
						sel = j == 0 ? 'selected' : '';
					timeTmp = t = t < 10?'0'+t:t;
					timeStr += '<li class="'+sel+'" data-time="'+t+':00">'+t+':00</li><li data-time="'+t+':30">'+t+':30</li>';
					if(j==option.timeNum - 1){
						timeStr += '<li class="'+sel+'" data-time="'+(t+1)+':00">'+(t+1)+':00</li>';
					}
				}
				timeStr += '<li></li><li></li>';
				
				mpTime.querySelector('ul').innerHTML += timeStr;
				
				
				var preventFun = function(e){
					e.preventDefault(); 
				}
				document.addEventListener('touchmove', preventFun, false);
				
				document.addEventListener('touchend', function (e) {
					document.removeEventListener('touchmove', preventFun,false);
				}, false);
				//初始化scroll
				var elHeight = itemHeight;
				
				var dateScroll = new IScroll('.date_select_box.active .mt_date', {
					snap : 'li',
					probeType : 2,
					tap : true
				});
				...
				dateScroll.on('scrollEnd', function(){
					...
					_this.updateSelectedEnd();
				});
				var timeScroll = new IScroll('.date_select_box.active .mt_time', {
					snap : 'li',
					probeType : 2,
					tap : true
				});
			...
			renderHTML : function(){
				...
				var html = '<div class="mt_body"><div class="mt_date"><ul><li class="mt_note">上下滚动选取时间</li><li></li></ul></div><div class="mt_time"><ul><li class="mt_note">可选时间：'+stime+'-'+etime+'</li><li></li></ul></div><div class="mt_indicate"></div></div>';
				document.querySelector('.timeslot_alert .date_select_box.active').innerHTML = html;
				
			},
			updateSelected : function(container, iscroll){
				
				var index = (-iscroll.y) / itemHeight + 2;
				var $containerLi = container.querySelectorAll('li');
				index = Math.round(index);

				$containerLi[index].className = 'selected';
				
				for(var i = 0; i < $containerLi.length;i++){
					if($containerLi[i].className == 'selected' && index != i){
						$containerLi[i].className = '';
					}
			...
			updateSelectedEnd : function(container, iscroll){
				
				$alertBox.querySelector('.alert_head a.active>span').innerText = document.querySelector('.date_select_box.active .mt_date .selected').getAttribute('data-date') + ' ' + document.querySelector('.date_select_box.active .mt_time .selected').getAttribute('data-time');
				
				
			},
			showPanel : function(container){
				var timeShow = document.querySelector(elements).querySelector('.date_time_show').innerText;
				var dateArr = timeShow.split(' ');
				var yearTime = dateArr[0].split('-');
				var startTime = yearTime[1] + '-' + yearTime[2] + ' ' + dateArr[1].split('~')[0];
				var endTime = yearTime[1] + '-' + yearTime[2] + ' ' + dateArr[1].split('~')[1];
				$alertBox.querySelector('#roomStart').innerText = startTime;
				$alertBox.querySelector('#roomEnd').innerText = endTime;
			...
			setValue : function(){
				var value = $alertBox.querySelector('.alert_head a.active>span').innerHTML;
				
				var dateArr = value.split(' '),
					date = dateArr[0],
					time = dateArr[1],
					$dateItem = document.querySelectorAll('.date_select_box.active .mt_date li'),
					$timeItem = document.querySelectorAll('.date_select_box.active .mt_time li'),
					dateItem,
					timeItem;
				
				for(var dateI = 0;dateI < $dateItem.length; dateI++){
					if(date == $dateItem[dateI].getAttribute('data-date')){
						dateItem = $dateItem[dateI - 2];
						break;
					}
				}
				for(var timeI = 0;timeI < $timeItem.length; timeI++){
					if(time == $timeItem[timeI].getAttribute('data-time')){
						timeItem = $timeItem[timeI - 2];
						break;
					}
				}
			...
			checkDate : function(el){
				if(el != undefined){
					this.dateScroll.scrollToElement(el);
				}
				
			},
			checkTime : function(el){
				if(el != undefined){
					this.timeScroll.scrollToElement(el);
				}
				
			}
		}
		
		$alertBox.querySelector('.ok_btn').addEventListener('click', function (e) {
			var $alertSpan = $alertBox.querySelectorAll('.alert_head>a>span');
			var startTime = $alertSpan[0].innerHTML;
			var endTime = $alertSpan[1].innerHTML;
			...
			if(option.filter){
				filterStatus = option.filter(startTime,endTime);
			}
			
			if(filterStatus){
				
				document.querySelector(elements).querySelector('.date_time_show').innerHTML = nowYear + '-' +startTime+'~'+endTimeArr[1];
				picker.hidePanel();
		...
		showHeadTime();
		function showHeadTime(){
			var nowTime = new Date(option.dateStart);
			console.log(nowTime)
			var nowTimeStart = nowTime.getTime();
			var blockShowHtml = '';
			var tmpYear = nowTime.getFullYear();
			var tmpMonth = nowTime.getMonth() + 1;
			var tmpDay = nowTime.getDate();
			var nowTimeSolt = 0;
			var roomsJsonTmp;
			//时间转为整数，例如14:00,14:30
			if(nowTime.Format('mm') > 30){
				nowTimeSolt = 60 - nowTime.Format('mm');
			}else if(nowTime.Format('mm') > 0){
				nowTimeSolt = 30 - nowTime.Format('mm');
			}
			
			nowTimeStart += nowTimeSolt*1000*60;
			
			//判断在可预订时间，自动跳转
			if(nowTime.Format('d') == new Date().Format('d')){//判断如果不是当前不自动跳转下一天
				
				if(nowTime.Format('hhmm') > (option.timeStart + option.timeNum + '00')){
					
					nowTimeStart = new Date(tmpYear+'/'+tmpMonth+'/'+(tmpDay+1)+' '+ option.timeStart +':00').getTime();
				}
			}else{
				nowTimeStart = new Date(tmpYear+'/'+tmpMonth+'/'+tmpDay+' '+ option.timeStart +':00').getTime();
			}
			
			nowTime = new Date(nowTimeStart);
			var nowTimeEnd = nowTimeStart + 1000*60*30;
			document.querySelector('.date_time_show').innerHTML = new Date(nowTimeStart).Format('yyyy-MM-dd hh:mm') + '~' + new Date(nowTimeEnd).Format('hh:mm');
		}
		
		
		
		$alertBox.querySelector('.cancel_btn').addEventListener('click', function (e) {
			picker.hidePanel();
		...
		//切换点击事件
		var $alertSwitch = $alertBox.querySelectorAll('.alert_head>a');
		for(var alertI = 0; alertI < $alertSwitch.length; alertI++){
			(function(i){
				$alertSwitch[i].onclick = function(){
					var $headClick = $alertBox.querySelectorAll(".alert_head>a");
					var $contentClick = $alertBox.querySelectorAll(".alert_date_main>.date_select_box");
					for(var n = 0;n < $headClick.length; n++){
						$headClick[n].className = '';
						$contentClick[n].className = $contentClick[n].className.replace('active','');
					}
					if($headClick[i].className != 'active'){
						$headClick[i].className = 'active';
						$contentClick[i].className += ' active';
						picker.dateScroll = null;
						picker.timeScroll = null;
						picker.init();
					}
				}
			})(alertI)
